...
mc <- function(R, r, num_points) {
  x1 <- runif(num_points, -R-r, R+r)
  x2 <- runif(num_points, -R-r, R+r)
...
a <- 0
b <- 1.2   #2x=6−3x⟹5x=6
c <- 0    #deci x=6/5, adica 1.2, iar y=2x=2.4 
d <- 2.4
n <- 20000
...
nr_puncte_inauntru <- 0
for (i in 1:n) {
  if (in_triunghi(x_puncte[i], y_puncte[i])) {
    nr_puncte_inauntru <- nr_puncte_inauntru + 1
  }
}
arie_triunghi <- arie_drept * (nr_puncte_inauntru / n)
...
#c)
estimate_probability<- function(total_years, epsilon, confidence_level) {
...
set.seed(123) 
result <- estimate_probability(total_years, epsilon, confidence_level)
