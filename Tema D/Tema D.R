...
data <- read.csv("probabilitati.csv", skip = 1, header = FALSE) #voi citi incepand
#de la al doilea rand pentru ca pe primul scrie "probabilitati", care nu este o val
#numerica si din cauza asta imi afiseaza NA
...
if (result1$z < -result1$z_critical) {
  cat("Nu a fost utilă la 1%.\n")
} else {
  cat("Nu se poate trage concluzia că nu a fost utilă 1%.\n")
}
...
if (result2$z < -result2$z_critical) {
  cat("Nu a fost utilă la 5%.\n")
} else {
  cat("Nu se poate trage concluzia că nu a fost utilă la 5%.\n")
}